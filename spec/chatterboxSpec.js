describe('chatterbox', function() {
  ...
  describe('chatroom behavior', function() {
    ...
    it('should be able to add rooms to the DOM', function() {
      ...
      // console.log($('#rooms select').children().length);
  ...
  describe('events', function() {
    it('should add a friend upon clicking their username', function() {
      sinon.spy(MessagesView, 'handleUsernameClick');
      ...
      expect(MessagesView.handleUsernameClick.called).to.be.true;

      MessagesView.handleUsernameClick.restore();
    });

    it('should add a room when clicking add', function() {
      sinon.spy( RoomsView, 'handleAddRoom');
      var prompt = window.prompt;
      window.prompt = sinon.stub().returns('testroom');
      App.initialize();
      $('#rooms').find('button').trigger('click');
      expect(RoomsView.handleAddRoom.called).to.be.true;

      window.prompt = prompt;
      RoomsView.handleAddRoom.restore();
    });
